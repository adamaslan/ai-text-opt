...
load_dotenv(dotenv_path=PROJECT_DIR.parent / ".env")  # chatbot1/.env
...
class ChromaRAGChatbot:
    ...
    def ask(self, question: str, top_k: Optional[int] = None) -> tuple[str, List[RetrievedDoc]]:
        """Returns (answer, retrieved_docs)."""
        docs = self._retriever.retrieve(question, top_k or self._top_k)
