...
def main():
    ...
    # Import datasets — load once, reuse for sample embeddings
    logger.info("\nImporting datasets...")
    loaded_data = {}
    for collection_name, file_path in EMBEDDING_FILES.items():
        ...
        if data:
            ...
            loaded_data[collection_name] = data
        else:
    ...
    # Example searches — reuse already-loaded data for sample vectors
    logger.info("\n" + "=" * 60)
    logger.info("Example Searches")
    logger.info("=" * 60)

    sample_embeddings = {}
    for collection_name, data in loaded_data.items():
